...
type T = { [K in keyof typeof translations.lt.trinkelesPas]: string };
  ...
  return (
    <section className="bg-atd-green pt-20 md:pt-24 pb-12 md:pb-16">
      <div className="max-w-5xl mx-auto px-6">
        ...
        <motion.div
          className="relative aspect-[21/9] mt-10 overflow-hidden"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.6, delay: 0.35 }}
        >
          <Image src="/images/trinkeles-plaza-1.jpg" alt={t.hero_img_alt} fill className="object-cover" sizes="100vw" priority />
        </motion.div>
      </div>
    </section>
  );
...
function Materials({ t }: { t: T }) {
  const mats = [
    { name: t.mat1_name, price: t.mat1_price, desc: t.mat1_desc },
    { name: t.mat2_name, price: t.mat2_price, desc: t.mat2_desc },
    { name: t.mat3_name, price: t.mat3_price, desc: t.mat3_desc },
    { name: t.mat4_name, price: t.mat4_price, desc: t.mat4_desc },
  ];
  return (
    <section className="bg-atd-cream py-16 md:py-20">
      <div className="max-w-5xl mx-auto px-6">
        <motion.div className="mb-10" {...fadeUp()}>
          <h2 className="font-display font-bold text-2xl md:text-3xl text-atd-black">{t.materials_heading}</h2>
          <p className="font-body text-atd-gray-soft mt-2">{t.materials_sub}</p>
        </motion.div>
        <div className="grid sm:grid-cols-2 gap-4">
          {mats.map((m, i) => (
            <motion.div key={i} className="bg-white border border-atd-black/10 p-5" {...fadeUp(i * 0.08)}>
              <div className="flex items-start justify-between gap-3">
                <h3 className="font-display font-bold text-lg text-atd-black leading-tight">{m.name}</h3>
                <span className="font-display font-bold text-atd-green text-base whitespace-nowrap shrink-0">{m.price}</span>
              </div>
              <p className="font-body text-sm text-atd-gray-soft mt-2 leading-relaxed">{m.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
          ...
          {items.map((item, i) => (
            <motion.div key={i} {...fadeUp(i * 0.05)}>
              <button type="button" aria-expanded={open === i}
                ...
                <span className="font-body font-semibold text-sm md:text-base text-atd-black leading-snug">{item.q}</span>
              ...
              <AnimatePresence initial={false}>
                {open === i && (
                  <motion.div key="a" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                    ...
                    <p className="font-body text-sm text-atd-gray-soft leading-relaxed pb-5">{item.a}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </motion.div>
  ...
  return (
    <section className="bg-atd-green py-16 md:py-20">
      <div className="max-w-5xl mx-auto px-6 text-center">
        <motion.div {...fadeUp()}>
          <h2 className="font-display font-bold text-2xl md:text-3xl text-white">{t.cta_heading}</h2>
          <p className="font-body text-white/70 mt-3 text-sm md:text-base max-w-md mx-auto">{t.cta_sub}</p>
        </motion.div>
        <motion.div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8" {...fadeUp(0.12)}>
          <a href="tel:+37065785096"
...
export function TrinkelesPas() {
  const { lang } = useLanguage();
  const t = translations[lang].trinkelesPas;
  return (
    <main>
      <Hero t={t} />
      <Materials t={t} />
